...
// ─── classifyRegime ──────────────────────────────────────────────
function testClassifyRegime() {
  console.log("\n=== classifyRegime — decision tree ===");
  ...
  // bypassing degenScore's timeframe normalization noise.
  const cutoffs = { slowCutoff: 15, hotCutoff: 45 };
  const fakeScored = (score) => [{ __score: score }];
  const classifyAt = (score) => {
    // classifyRegime always recomputes degenScore internally, so to test the
    // pure threshold logic in isolation we mirror its branch structure here
    // with a known aggregateScore rather than reverse-engineering pool fields
    // that produce an exact score.
...
function testRegimeLibrary() {
  console.log("\n=== market-regime-library — profiles + active pointer ===");
  const before = fs.existsSync(REGIME_FILE) ? fs.readFileSync(REGIME_FILE, "utf8") : null;
  ...
  try {
    ...
    if (before !== null) fs.writeFileSync(REGIME_FILE, before);
    console.log("  (restored market-regime-profiles.json to pre-test content)");
  }
}

// ─── applyConfigChanges (shared with update_config) ──────────────
function testApplyConfigChanges() {
  console.log("\n=== applyConfigChanges — regime profile hot-apply ===");
  const before = fs.readFileSync(USER_CONFIG_FILE, "utf8");
  ...
  try {
    ...
    fs.writeFileSync(USER_CONFIG_FILE, before);
